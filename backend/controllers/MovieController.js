...
class MovieController {
    ...
    edit(req, res, next) {
        Movie.findOneAndUpdate({ biDanh: req.params.bidanh }, req.body, { runValidator: true })
            .then(data => {
                if (data) {
    ...
    top20Movies(req, res, next) {
        Movie.find({ soLuongBan: { $gt: 0 } })
            ...
            .then((data) => {
                if (data.length > 0)
                    res.status(200).json({ message: 'Top 20 phim được xem nhiều của rạp', top10: data })
                else res.status(200).json({ message: 'Top 20 phim được xem nhiều của rạp', top10: 'Chưa có sách nào được bán' })
            })
            .catch(err => {
    ...
    async top20Showtimes(req, res, next) {
        const ticket = await TicketBooking.find({})
